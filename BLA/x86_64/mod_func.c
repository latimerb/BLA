#include <stdio.h>
#include "hocdec.h"
extern int nrnmpi_myid;
extern int nrn_nobanner_;

extern void _bg2pyr_reg(void);
extern void _cadyn_reg(void);
extern void _cal2_reg(void);
extern void _ca_reg(void);
extern void _capool_reg(void);
extern void _function_TMonitor_reg(void);
extern void _gap_reg(void);
extern void _Gfluct_new_exc_reg(void);
extern void _Gfluct_new_inh_reg(void);
extern void _h_reg(void);
extern void _im_reg(void);
extern void _interD2interD_STFD_new_reg(void);
extern void _interD2pyrD_STFD_new_reg(void);
extern void _kadist_reg(void);
extern void _kaprox_reg(void);
extern void _kdrca1DA_reg(void);
extern void _kdrca1_reg(void);
extern void _kdrinter_reg(void);
extern void _leakDA_reg(void);
extern void _leakinter_reg(void);
extern void _leak_reg(void);
extern void _na3DA_reg(void);
extern void _na3_reg(void);
extern void _nainter_reg(void);
extern void _nap_reg(void);
extern void _nat_reg(void);
extern void _pyrD2interD_STFD_reg(void);
extern void _pyrD2pyrD_STFD_new_reg(void);
extern void _sahp_reg(void);
extern void _sahpNE_reg(void);
extern void _vecevent_reg(void);
extern void _xtra_imemrec_reg(void);
extern void _xtra_reg(void);

void modl_reg(){
  if (!nrn_nobanner_) if (nrnmpi_myid < 1) {
    fprintf(stderr, "Additional mechanisms from files\n");

    fprintf(stderr," mod_nsgportal/bg2pyr.mod");
    fprintf(stderr," mod_nsgportal/cadyn.mod");
    fprintf(stderr," mod_nsgportal/cal2.mod");
    fprintf(stderr," mod_nsgportal/ca.mod");
    fprintf(stderr," mod_nsgportal/capool.mod");
    fprintf(stderr," mod_nsgportal/function_TMonitor.mod");
    fprintf(stderr," mod_nsgportal/gap.mod");
    fprintf(stderr," mod_nsgportal/Gfluct_new_exc.mod");
    fprintf(stderr," mod_nsgportal/Gfluct_new_inh.mod");
    fprintf(stderr," mod_nsgportal/h.mod");
    fprintf(stderr," mod_nsgportal/im.mod");
    fprintf(stderr," mod_nsgportal/interD2interD_STFD_new.mod");
    fprintf(stderr," mod_nsgportal/interD2pyrD_STFD_new.mod");
    fprintf(stderr," mod_nsgportal/kadist.mod");
    fprintf(stderr," mod_nsgportal/kaprox.mod");
    fprintf(stderr," mod_nsgportal/kdrca1DA.mod");
    fprintf(stderr," mod_nsgportal/kdrca1.mod");
    fprintf(stderr," mod_nsgportal/kdrinter.mod");
    fprintf(stderr," mod_nsgportal/leakDA.mod");
    fprintf(stderr," mod_nsgportal/leakinter.mod");
    fprintf(stderr," mod_nsgportal/leak.mod");
    fprintf(stderr," mod_nsgportal/na3DA.mod");
    fprintf(stderr," mod_nsgportal/na3.mod");
    fprintf(stderr," mod_nsgportal/nainter.mod");
    fprintf(stderr," mod_nsgportal/nap.mod");
    fprintf(stderr," mod_nsgportal/nat.mod");
    fprintf(stderr," mod_nsgportal/pyrD2interD_STFD.mod");
    fprintf(stderr," mod_nsgportal/pyrD2pyrD_STFD_new.mod");
    fprintf(stderr," mod_nsgportal/sahp.mod");
    fprintf(stderr," mod_nsgportal/sahpNE.mod");
    fprintf(stderr," mod_nsgportal/vecevent.mod");
    fprintf(stderr," mod_nsgportal/xtra_imemrec.mod");
    fprintf(stderr," mod_nsgportal/xtra.mod");
    fprintf(stderr, "\n");
  }
  _bg2pyr_reg();
  _cadyn_reg();
  _cal2_reg();
  _ca_reg();
  _capool_reg();
  _function_TMonitor_reg();
  _gap_reg();
  _Gfluct_new_exc_reg();
  _Gfluct_new_inh_reg();
  _h_reg();
  _im_reg();
  _interD2interD_STFD_new_reg();
  _interD2pyrD_STFD_new_reg();
  _kadist_reg();
  _kaprox_reg();
  _kdrca1DA_reg();
  _kdrca1_reg();
  _kdrinter_reg();
  _leakDA_reg();
  _leakinter_reg();
  _leak_reg();
  _na3DA_reg();
  _na3_reg();
  _nainter_reg();
  _nap_reg();
  _nat_reg();
  _pyrD2interD_STFD_reg();
  _pyrD2pyrD_STFD_new_reg();
  _sahp_reg();
  _sahpNE_reg();
  _vecevent_reg();
  _xtra_imemrec_reg();
  _xtra_reg();
}
